...
void FileBuffer::CopyFrom(FileBuffer *buf, const unsigned int n)
{
...
void FileBuffer::CopyTo(FileBuffer *buf, const unsigned int n)
{
    if (buffer && n && (current + n <= buffer + size))
    {
...
void FileBuffer::Fill(FileBuffer *buf)
{
    if (buffer)
    {
        current = buffer;
...
void FileBuffer::Load(std::ifstream &ifs)
{
    if (ifs.is_open())
    {
        current = buffer;
...
void FileBuffer::Save(std::ofstream &ofs)
{
    if (ofs.is_open())
    {
        current = buffer;
...
void FileBuffer::Save(std::ofstream &ofs, const unsigned int n)
{
    if (ofs.is_open())
    {
        if (n <= size)
...
void FileBuffer::Dump(const unsigned int n)
{
    uint8_t *tmp = current;
...
void FileBuffer::Seek(const unsigned int n)
{
    if ((current) && (n <= size))
    {
        current = buffer + n;
...
void FileBuffer::Skip(const int n)
{
...
void FileBuffer::GetData(void *data,
...
void FileBuffer::PutUint8(const uint8_t x)
{
    uint8_t xx = x;
    PutData(&xx, 1);
}

void FileBuffer::PutUint16LE(const uint16_t x)
{
    uint16_t xx = SDL_SwapLE16(x);
    PutData(&xx, 2);
}

void FileBuffer::PutUint16BE(const uint16_t x)
{
    uint16_t xx = SDL_SwapBE16(x);
    PutData(&xx, 2);
}

void FileBuffer::PutUint32LE(const uint32_t x)
{
    uint32_t xx = SDL_SwapLE32(x);
    PutData(&xx, 4);
}

void FileBuffer::PutUint32BE(const uint32_t x)
{
    uint32_t xx = SDL_SwapBE32(x);
    PutData(&xx, 4);
}

void FileBuffer::PutSint8(const int8_t x)
{
    int8_t xx = x;
    PutData(&xx, 1);
}

void FileBuffer::PutSint16LE(const int16_t x)
{
    int16_t xx = SDL_SwapLE16(x);
    PutData(&xx, 2);
}

void FileBuffer::PutSint16BE(const int16_t x)
{
    int16_t xx = SDL_SwapBE16(x);
    PutData(&xx, 2);
}

void FileBuffer::PutSint32LE(const int32_t x)
{
    int32_t xx = SDL_SwapLE32(x);
    PutData(&xx, 4);
}

void FileBuffer::PutSint32BE(const int32_t x)
{
    int32_t xx = SDL_SwapBE32(x);
    PutData(&xx, 4);
}

void FileBuffer::PutString(const std::string s)
{
...
void FileBuffer::PutString(const std::string s, const unsigned int len)
{
    if ((current) && (current + len <= buffer + size))
    {
...
void FileBuffer::PutData(void *data, const unsigned int n)
{
    if (current + n <= buffer + size)
    {
